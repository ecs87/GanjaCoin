...
{
    // Convert Ganjaproject:// to Ganjaproject:
    //
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Ganjaproject.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Ganjaproject.lnk
...
{
    return GetAutostartDir() / "Ganjaproject.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Ganjaproject.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Ganjaproject\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Ganjaproject app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Ganjaproject app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Ganjaproject-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Ganjaproject-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Ganjaproject-Qt"));
