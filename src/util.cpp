...
#else
    const char* pszModule = "Ganjaproject2";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Ganjaproject
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Ganjaproject
    // Mac: ~/Library/Application Support/Ganjaproject
    // Unix: ~/.Ganjaproject
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ganjaproject2";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Ganjaproject.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "Ganjaproject.conf";
               ...
               fprintf(ConfFile, "port=12419\n");
               fprintf(ConfFile, "rpcport=12420\n");
               fprintf(ConfFile, "rpcconnect=127.0.0.1\n");
               fprintf(ConfFile, "rpcallowip=127.0.0.1\n");
               fprintf(ConfFile, "addnode=138.197.44.71\n");
              // fprintf(ConfFile, "masternodeminprotocol=60030\n");
