...
const string strMessageMagic = "Ganjaproject Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight == 1 )
       {
            nSubsidy = 39500000 * COIN; // SWAP
        }
    else if (nHeight >= 2 && nHeight <= 40000)
       {
            nSubsidy = 0.1 * COIN;
       }
    else
...
int64_t GetProofOfStakeReward(int nHeight, int64_t nCoinAge, int64_t nFees)
{
    ...
    if(nHeight >= 1 && nHeight <= 20000) //
    {
      nSubsidy = 15 * COIN; //300,000 in rewards
    }
     else if (nHeight >= 20001 && nHeight <= 30000)
    {
         nSubsidy = 10 * COIN; // 99,999 in rewards
    }
 else if (nHeight >= 30001 && nHeight <= 40000)
    {
         nSubsidy = 5 * COIN; // 49995 in rewards
    }
    else
      {
        nSubsidy = 3 * COIN;
...
{
    RenameThread("Ganjaproject-loadblk");
